...
parser.add_argument('--cfg', type=str, default= '../experiments/carsda_r50_l234/config.yaml',
...
def build_opt_lr(model, current_epoch=0):
    ...
    if cfg.ALIGN.ALIGN:
        trainable_params += [{'params': model.align.parameters(),
                              'lr': cfg.TRAIN.BASE_LR_ali}]
    
    trainable_params += [{'params': model.projection.parameters(),
                          'lr': cfg.TRAIN.BASE_LR_ali}]
    trainable_params += [{'params': model.predictor.parameters(),
                          'lr': cfg.TRAIN.BASE_LR_lin}]
...
def train(train_loader, model, optimizer, lr_scheduler, tb_writer):
    ...
    for idx, data in enumerate(train_loader):
        ...
        # print("Number of parameter: %.2fM" % (total_trainable_params/1e6))
