...
from siamban.models.neck import get_neck, Projection, Predictor
import numpy as np
import cv2
...
class ModelBuilder(nn.Module):
    ...
    def forward(self, data):
        ...
        cls_copy, loc_copy = self.head(zf_copy, xf_copy)

        # get loss
        # cls loss with cross entropy loss
        cls_copy = self.log_softmax(cls_copy)
        cls_loss_copy = select_cross_entropy_loss(cls_copy, label_cls)

        # loc loss with iou loss
        loc_loss_copy = select_iou_loss(loc_copy, label_loc, label_cls)
        ...
        outputs['total_loss'] = cfg.TRAIN.CLS_WEIGHT * cls_loss + \
            cfg.TRAIN.LOC_WEIGHT * loc_loss + cfg.TRAIN.DA_WEIGHT * da_loss
